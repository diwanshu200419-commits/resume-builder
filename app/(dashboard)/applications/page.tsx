...
const LINKEDIN_RECOMMENDATIONS = [
  ...
  },
  {
    ...
    linkedinUrl: "https://www.linkedin.com/jobs/view/uber-frontend-architect",
  },
  {
    id: "rec-3",
    ...
    skills: ["Next.js 14", "Node.js", "Vector DBs"],
    linkedinUrl: "https://www.linkedin.com/jobs/view/notion-fullstack-ai",
  },
  {
    id: "rec-4",
    ...
    linkedinUrl: "https://www.linkedin.com/jobs/view/zomato-eng-manager",
  },
];
...
export default function KanbanBoardPage() {
  ...
  const handleToggleJobAlert = (jobId: string, roleTitle: string, companyName: string) => {
    ...
    if (newStatus) {
      setAlertSuccessToast(`🔔 Job Alert Active: You will receive daily matching alerts for "${roleTitle}" at ${companyName}!`);
  ...
  const handleImportRecommended = (rec: (typeof LINKEDIN_RECOMMENDATIONS)[0]) => {
    const importedApp: Application = {
      ...
      notes: `Imported from LinkedIn feed. Matching skills: ${rec.skills.join(", ")}`,
      appliedDate: new Date().toISOString().slice(0, 10),
    ...
    <div className="max-w-7xl mx-auto py-6 sm:py-10 space-y-8 text-text-primary">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold flex items-center gap-2 tracking-tight">
            Job Tracker &amp; AI Alerts Dashboard
          </h1>
          <p className="text-text-secondary text-sm mt-1">
            Track job applications across recruitment pipelines and configure 1-click real-time AI job alerts for every posting.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {STAGES.map((stg) => {
          ...
          return (
            <div key={stg.key} className="space-y-3 bg-surface-elevated/40 border border-border p-3.5 rounded-2xl flex flex-col min-h-[450px]">
                  ...
                  stageApps.map((app) => (
                    ...
                    >
                      ...
                      <div className="pt-2 border-t border-border flex items-center justify-between text-[10px] text-text-muted">
                        ...
                        <span className="text-indigo-500 font-bold hover:underline">View Details →</span>
                      </div>
                    </Card>
                  ))
                )}
      ...
      {/* Recommended Tech Roles Feed WITH 1-CLICK JOB ALERTS */}
      <div className="space-y-6 pt-6 border-t border-border">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <h2 className="text-xl font-bold text-text-primary flex items-center gap-2">
              <Linkedin className="w-5 h-5 text-sky-500" />
              Recommended Tech Roles (Live Feed &amp; Job Alerts)
            </h2>
            <p className="text-xs text-text-secondary">Click &apos;Set Job Alert&apos; on any role to receive real-time match notifications.</p>
          </div>
          <Badge variant="outline" className="text-xs font-semibold border-sky-500/30 text-sky-400 w-max">
            1-Click Job Alerts Active
          </Badge>
        </div>
