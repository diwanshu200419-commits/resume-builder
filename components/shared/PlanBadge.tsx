...
const planConfig: Record<string, { label: string; variant: "default" | "success" | "warning"; icon: React.ReactNode }> = {
  ...
  career_pack: { label: "Career Pack", variant: "warning", icon: <Crown className="w-3 h-3" /> },
};

const fallbackConfig = { label: "Free", variant: "default" as const, icon: <Zap className="w-3 h-3" /> };

export function PlanBadge({ plan }: { plan: Plan | string }) {
  const config = planConfig[plan] ?? fallbackConfig;
  return (
